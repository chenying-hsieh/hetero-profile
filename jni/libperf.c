...
unsigned long long libperf_get_info(struct libperf_data *pd, void *id)
{
  int result, nr_counters = __LIBPERF_ARRAY_SIZE(default_attrs);
  int *fds = pd->fds;
  uint64_t count[3];                 /* potentially 3 values */
  int i = *(int *)id;
  ...
  update_stats(&event_stats[i], count[0]);
  return (unsigned long long)count[0];
}
